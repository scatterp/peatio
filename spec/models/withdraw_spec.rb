...
describe Withdraw do
  ...
  context '#fix_precision' do
    it "should trucate values to max precision" do
      YmlCurrency.where(code: :btc).first.precision = 5
      withdraw = create(:satoshi_withdraw, sum: 0.123456789)
      expect(withdraw.sum).to eq 0.12345
    end
  end
  ...
  context 'coin withdraw' do
    ...
    describe 'sn' do
      ...
      it 'alias withdraw_id to sn' do
        expect(@withdraw.withdraw_id).to eq(@withdraw.sn)
      end
    end

    it "won't accept account_id from outside" do
      withdrawal = build :satoshi_withdraw, account_id: 999
      withdrawal.save!

      expect(withdrawal.account_id).to eq(withdrawal.member.get_account(withdrawal.currency).id)
    end
  end
  ...
  context 'Worker::WithdrawCoin#process' do
    ...
    it 'transitions to :almost_done after calling rpc but getting Exception' do
      ...
      expect(lambda { Worker::WithdrawCoin.new.process({id: subject.id}, {}, {}) }).to raise_error(Account::BalanceError)
